...
let acStrictSeq = 0;
function ACStrict({ value, onChange, options, placeholder = "", style }) {
    ...
    <>
      <input
        ...
        onChange={(e) => {
          ...
          if (exact) onChange(exact); // selectie valida -> propagam imediat (auto-fill CodSAGA etc.)
        }}
        onBlur={() => {
          const exact = findExact(q);
          if (exact) { if (exact !== q) setQ(exact); }
          else setQ(value || ""); // text invalid -> revenim silentios
        }}
      />
