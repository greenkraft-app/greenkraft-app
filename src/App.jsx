...
// ── Autocomplete ──────────────────────────────────────────────
function AC({ value, onChange, options, placeholder = "", strict = false }) {
  ...
  const handleBlur = () => {
    setTimeout(() => {
      setOpen(false);
      if (!strict) return;
      // Modul strict: materialul TREBUIE sa existe in lista (Variabile). Daca la iesirea din
      // camp textul nu se potriveste exact cu niciun produs, revenim la ultima valoare valida —
      // altfel ramane un denumire "orfan", fara CodSAGA, care strica raportarea.
      const exact = options.find((o) => o.toLowerCase() === (q || "").toLowerCase());
      if (exact) { if (exact !== q) { setQ(exact); onChange(exact); } }
      else { setQ(value || ""); if ((q || "") !== (value || "")) onChange(value || ""); }
    }, 150);
  };
  return (
    <div style={{ position: "relative" }}>
      <input ref={inputRef} title={q || undefined} style={inp({ textAlign: "center" })} value={q} placeholder={placeholder}
        onChange={(e) => { setQ(e.target.value); if (!strict) onChange(e.target.value); updatePos(); setOpen(true); }}
        ...
        <div onClick={() => setTicEdit(null)} style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.55)", zIndex: 9999, display: "flex", alignItems: "center", justifyContent: "center", padding: 16 }}>
          <div onClick={(e) => e.stopPropagation()} style={{ background: "#fff", borderRadius: 10, width: "min(96vw,440px)", display: "flex", flexDirection: "column", boxShadow: "0 8px 40px rgba(0,0,0,0.35)", overflow: "hidden" }}>
            ...
            <div style={{ padding: "14px 18px", display: "flex", flexDirection: "column", gap: 10, maxHeight: "70vh", overflowY: "auto" }}>
              ...
              <div><label style={{ fontSize: 11, fontWeight: 600, color: "#555" }}>Material / Deșeu</label><div style={{ border: "1px solid #ccc", borderRadius: 5, padding: "2px 4px" }}><AC value={ticEdit.material} options={PRODUSE_DYN} placeholder="Selectează..." strict onChange={(v) => setTicEdit((p) => ({ ...p, material: v }))} /></div></div>
                        ...
                        <table style={{ borderCollapse: "collapse", width: "100%" }}>
                          ...
                          <tbody>{pv.materiale.map((m, i) => (
                            <tr key={i} style={{ background: i % 2 === 0 ? "#fff" : "#fffde7" }}>
                              <td style={td()}><AC value={m.den} options={PRODUSE_DYN} placeholder="Selectează..." strict onChange={(v) => updPVMat(i, "den", v)} /></td>
                ...
                <div style={{ maxWidth: 860 }}>
                  ...
                  <div style={{ display: "flex", gap: 14, flexWrap: "wrap" }}>
                    ...
                    <div style={{ flex: "1 1 380px", background: "#fff", border: "1px solid #e0e0e0", borderRadius: 10, padding: 16 }}>
                      ...
                      <div style={{ marginBottom: 10 }}><label style={FL}>Material / Deșeu</label><div style={ACB}><AC value={ticNou.material} options={PRODUSE_DYN} placeholder="Selectează..." strict onChange={(v) => setTicNou((p) => ({ ...p, material: v }))} /></div></div>
                      <div style={{ display: "flex", gap: 8 }}>
