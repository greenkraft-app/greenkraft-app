...
const inp = (x = {}) => ({ width: "100%", border: "none", outline: "none", fontSize: 12, background: "transparent", textOverflow: "ellipsis", overflow: "hidden", whiteSpace: "nowrap", ...x });
// Pentru campuri numerice (cantitati, preturi): FARA ellipsis — o cifra trunchiata poate fi citita gresit
const inpNum = (x = {}) => ({ width: "100%", border: "none", outline: "none", fontSize: 12, background: "transparent", ...x });
const sel = (x = {}) => ({ border: "none", outline: "none", background: "transparent", fontSize: 12, width: "100%", cursor: "pointer", textOverflow: "ellipsis", overflow: "hidden", whiteSpace: "nowrap", ...x });
...
function AC({ value, onChange, options, placeholder = "" }) {
  ...
  return (
    <div style={{ position: "relative" }}>
      <input ref={inputRef} title={q || undefined} style={inp({ textAlign: "center" })} value={q} placeholder={placeholder}
...
export default function App() {
  ...
  const a3FirmaInfo = (denumire) => {
    ...
    return f ? { cui: f.cod_fiscal || "", adresa: f.adresa || "", ...(f.anexa3 || {}) } : null;
  };
                    ...
                    {matTipiceModal.items.map((it, ai) => (
                      <tr key={ai} style={{ background: ai % 2 === 0 ? "#fff" : "#fff8f5" }}>
                        <td style={{ padding: "4px 6px" }}>
                          <select
                            style={{ width: "100%", padding: "5px 6px", border: "1px solid #ddd", borderRadius: 4, fontSize: 12, textAlign: "center", boxSizing: "border-box", cursor: "pointer" }}
                            value={it.den}
                            onChange={(e) => {
                          ...
                          >
                            ...
                            {PRODUSE_DYN.map((o) => <option key={o} value={o}>{o}</option>)}
                          </select>
                        </td>
                        <td style={{ padding: "4px 6px" }}>
                          <input
        ...
        <div onClick={() => setTicEdit(null)} style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.55)", zIndex: 9999, display: "flex", alignItems: "center", justifyContent: "center", padding: 16 }}>
          <div onClick={(e) => e.stopPropagation()} style={{ background: "#fff", borderRadius: 10, width: "min(96vw,440px)", display: "flex", flexDirection: "column", boxShadow: "0 8px 40px rgba(0,0,0,0.35)", overflow: "hidden" }}>
            ...
            <div style={{ padding: "14px 18px", display: "flex", flexDirection: "column", gap: 10, maxHeight: "70vh", overflowY: "auto" }}>
              ...
              <div><label style={{ fontSize: 11, fontWeight: 600, color: "#555" }}>Material / Deșeu</label><div style={{ border: "1px solid #ccc", borderRadius: 5, padding: "2px 4px" }}><select style={{ width: "100%", padding: "6px 8px", border: "1px solid #ccc", borderRadius: 5, fontSize: 13, boxSizing: "border-box", cursor: "pointer" }} value={ticEdit.material} onChange={(e) => setTicEdit((p) => ({ ...p, material: e.target.value }))}><option value="">Selectează...</option>{PRODUSE_DYN.map((o) => <option key={o} value={o}>{o}</option>)}</select></div></div>
      ...
      <div style={{ background: "#fff", border: "1px solid #ccc", borderTop: "none", borderRadius: "0 0 8px 8px", padding: 14, boxShadow: "0 2px 8px rgba(0,0,0,.08)" }}>
                    ...
                    <div style={{ flex: 1, minWidth: 260 }}>
                      <div style={{ background: "#fff8e1", border: "1px solid #ffd54f", borderRadius: 8, padding: 12, marginBottom: 10 }}>
                        ...
                        <table style={{ borderCollapse: "collapse", width: "100%" }}>
                          <thead><tr><th style={th({ textAlign: "left", background: "#e65100", minWidth: 170 })}>Denumire</th><th style={th({ width: 85, background: "#e65100" })}>CodSAGA</th><th style={th({ width: 96, background: "#e65100" })}>Cant.(kg)</th><th style={th({ width: 68, background: "#e65100" })}>Preț</th><th style={th({ width: 72, background: "#e65100" })}>Valoare</th><th style={th({ width: 26, background: "#e65100" })}></th></tr></thead>
                          <tbody>{b.produse.map((p, i) => { const v = (parseFloat(p.cant) || 0) * (parseFloat(p.pret) || 0); return (<tr key={i} style={{ background: i % 2 === 0 ? "#fff" : "#fffde7" }}><td style={td()}><select style={sel({ textAlign: "center" })} value={p.den} onChange={(e) => updP(i, "den", e.target.value)}><option value="">Selectează...</option>{PRODUSE_DYN.map((o) => <option key={o} value={o}>{o}</option>)}</select></td><td style={td()}><input style={inp({ textAlign: "center" })} value={p.cod_art || ""} onChange={(e) => updP(i, "cod_art", e.target.value)} /></td><td style={td()}><div style={{ display: "flex", gap: 2, alignItems: "center" }}><input style={inpNum({ textAlign: "right", minWidth: 34 })} type="number" value={p.cant} onChange={(e) => updP(i, "cant", e.target.value)} />{scalePort && <button onClick={() => useScaleWeight(v => updP(i, "cant", v))} title="Citește din cantar" style={{ background: scaleReading?.stable ? "#e8f5e9" : "#fff8e1", border: "1px solid #ccc", borderRadius: 3, padding: "1px 3px", cursor: "pointer", fontSize: 10, flexShrink: 0 }}>⚖️</button>}</div></td><td style={td()}><input style={inpNum({ textAlign: "right" })} type="number" value={p.pret} onChange={(e) => updP(i, "pret", e.target.value)} /></td><td style={td({ textAlign: "right", fontWeight: 600, background: "#fff8e1" })}>{v > 0 ? fmt(v) : "—"}</td><td style={td({ textAlign: "center", padding: 2 })}><button onClick={() => setB((b) => ({ ...b, produse: b.produse.filter((_, j) => j !== i) }))} style={{ background: "none", border: "none", cursor: "pointer", color: "#e53935", fontSize: 13 }}>✕</button></td></tr>); })}</tbody>
                        </table>
        ...
        {tab === "pv" && (
          <div>
                  ...
                  <div style={{ display: "flex", gap: 14, flexWrap: "wrap" }}>
                    ...
                    <div style={{ flex: 1, minWidth: 320 }}>
                      <div style={{ background: "#fff8e1", border: "1px solid #ffd54f", borderRadius: 8, padding: 12 }}>
                        ...
                        <table style={{ borderCollapse: "collapse", width: "100%" }}>
                          <thead><tr><th style={th({ textAlign: "left", background: "#e65100", minWidth: 200 })}>Denumire</th><th style={th({ width: 85, background: "#e65100" })}>CodSAGA</th><th style={th({ width: 96, background: "#e65100" })}>Cant.(kg)</th><th style={th({ width: 26, background: "#e65100" })}></th></tr></thead>
                          <tbody>{pv.materiale.map((m, i) => (
                            <tr key={i} style={{ background: i % 2 === 0 ? "#fff" : "#fffde7" }}>
                              <td style={td()}><select style={sel({ textAlign: "center" })} value={m.den} onChange={(e) => updPVMat(i, "den", e.target.value)}><option value="">Selectează...</option>{PRODUSE_DYN.map((o) => <option key={o} value={o}>{o}</option>)}</select></td>
            ...
            {pvSubTab === "anexa3" && (() => {
              const firmeOpts = pjList.map((f) => f.denumire).filter(Boolean);
                      ...
                      <div style={{ display: "flex", gap: 14, flexWrap: "wrap" }}>
                        <div style={{ flex: "1 1 420px", background: "#fff", border: "1px solid #e0e0e0", borderRadius: 10, padding: 16 }}>
                          ...
                          <div style={{ marginBottom: 10 }}>
                            ...
                            <div style={ACB}><AC value={a3Nou.transportator} options={firmeOpts} placeholder="Caută firmă..." onChange={(v) => setA3Nou((p) => ({ ...p, transportator: v }))} /></div>
                            {Preview(trInfo)}
                          </div>
                          <div style={{ display: "flex", gap: 8, marginBottom: 10 }}>
                          ...
                          <div style={{ marginBottom: 10 }}>
                            ...
                            <div style={ACB}><AC value={a3Nou.categorie} options={ANEXA3_CATEGORII} placeholder="Selectează sau scrie..." onChange={(v) => setA3Nou((p) => ({ ...p, categorie: v }))} /></div>
                          </div>
                          <div style={{ marginBottom: 10 }}>
                            <label style={FL}>Expeditor (cine predă marfa) <span style={{ color: "#c62828" }}>*</span></label>
                            <div style={ACB}><AC value={a3Nou.expeditor} options={firmeOpts} placeholder="Caută firmă..." onChange={(v) => setA3Nou((p) => ({ ...p, expeditor: v }))} /></div>
                            {Preview(expInfo)}
                          </div>
                          <div style={{ marginBottom: 10 }}>
                            <label style={FL}>Destinatar (cine primește marfa) <span style={{ color: "#c62828" }}>*</span></label>
                            <div style={ACB}><AC value={a3Nou.destinatar} options={firmeOpts} placeholder="Caută firmă..." onChange={(v) => setA3Nou((p) => ({ ...p, destinatar: v }))} /></div>
                            {Preview(destInfo)}
                          </div>
                          <div style={{ display: "flex", gap: 8 }}>
                            <div style={{ flex: 1 }}>
          ...
          <div>
            ...
            <div style={{ overflowX: "auto" }}>
              <table style={{ borderCollapse: "collapse", width: "100%" }}>
                ...
                <tbody>{chFiltered.map((r, idx) => { const i = chRows.indexOf(r); const rowBg = idx % 2 === 0 ? "#fff" : "#f7faf8"; const achBg = r.ach === "Da" ? "#e8f5e9" : r.ach === "Nu" ? "#ffebee" : "#fff"; const achC = r.ach === "Da" ? G : r.ach === "Nu" ? "#c62828" : "#555"; return (<tr key={r.id || i} style={{ background: rowBg }}><td style={td({ textAlign: "center", color: "#aaa", fontSize: 10, background: "#f5f5f5" })}>{idx + 2}</td><td style={td({ background: rowBg })}><DateInput value={r.data || ""} onChange={(v) => updCH(i, "data", v)} /></td><td style={td({ background: "#fffde7" })}><select style={sel({ textAlign: "center" })} value={r.gk || ""} onChange={(e) => updCH(i, "gk", e.target.value)}>{GREENKRAFT_OPT.map((o) => <option key={o}>{o}</option>)}</select></td><td style={td({ background: rowBg })}><input style={inp({ textAlign: "right" })} value={r.suma || ""} onChange={(e) => updCH(i, "suma", e.target.value)} /></td><td style={td({ background: "#fffde7" })}><select style={sel({ textAlign: "center" })} value={r.cat || ""} onChange={(e) => updCH(i, "cat", e.target.value)}>{CATEGORIE_CH.map((o) => <option key={o}>{o}</option>)}</select></td><td style={{ ...td({ background: rowBg }), overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}><input title={r.det || undefined} style={inp({ textAlign: "center" })} value={r.det || ""} onChange={(e) => updCH(i, "det", e.target.value)} /></td><td style={td({ background: achBg })}><select style={sel({ color: achC, fontWeight: 700, textAlign: "center" })} value={r.ach || ""} onChange={(e) => updCH(i, "ach", e.target.value)}><option value=""></option><option>Da</option><option>Nu</option></select></td><td style={td({ background: "#fffde7" })}><AC value={r.ach_de || ""} options={achitatOptions} onChange={(v) => updCH(i, "ach_de", v)} placeholder="—" /></td><td style={{ ...td({ background: rowBg }), overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}><input title={r.note || undefined} style={inp({ textAlign: "center" })} value={r.note || ""} onChange={(e) => updCH(i, "note", e.target.value)} /></td><td style={td({ textAlign: "center", padding: 3 })}><button onClick={() => delCH(r.id)} style={{ background: "none", border: "none", cursor: "pointer", color: "#e53935", fontSize: 13 }}>✕</button></td></tr>); })}</tbody>
          ...
          <div>
            ...
            <div style={{ overflowX: "auto" }}>
              <table style={{ borderCollapse: "collapse", width: "100%", tableLayout: "fixed" }}>
                ...
                <tbody>{livFiltered.map((r, idx) => { const i = livRows.indexOf(r); const tot = (parseFloat(r.cant) || 0) * (parseFloat(r.pret) || 0); const rowBg = idx % 2 === 0 ? "#fff" : "#f8fbf9"; return (<tr key={r.id || i} style={{ background: rowBg }}><td style={td({ textAlign: "center", color: "#aaa", fontSize: 10, background: "#f5f5f5" })}>{idx + 2}</td><td style={td({ background: rowBg })}><DateInput value={r.data || ""} onChange={(v) => updLIV(i, "data", v)} /></td><td style={td({ background: rowBg })}><input style={inp({ textAlign: "center" })} value={r.nr || ""} onChange={(e) => updLIV(i, "nr", e.target.value)} /></td><td style={td({ background: "#fffde7" })}><AC value={r.client || ""} options={clientOptions} onChange={(v) => updLIV(i, "client", v)} placeholder="—" /></td><td style={{ ...td({ background: rowBg }), overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }} title={r.produs}><select style={sel({ textAlign: "center" })} value={r.produs || ""} onChange={(e) => updLIV(i, "produs", e.target.value)}><option value="">—</option>{PRODUSE_DYN.map((o) => <option key={o} value={o}>{o}</option>)}</select></td><td style={td({ background: rowBg })}><input style={inpNum({ textAlign: "right" })} type="number" value={r.cant || ""} onChange={(e) => updLIV(i, "cant", e.target.value)} /></td><td style={td({ background: rowBg })}><input style={inpNum({ textAlign: "right" })} type="number" value={r.pret || ""} onChange={(e) => updLIV(i, "pret", e.target.value)} /></td><td style={td({ textAlign: "right", background: "#f0f4f0", fontWeight: 600 })}>{tot > 0 ? fmt(tot) : "0,00"}</td><td style={td({ background: r.fact === "DA" ? "#e8f5e9" : r.fact === "NU" ? "#ffebee" : "#fff", textAlign: "center" })}><select style={sel({ color: r.fact === "DA" ? G : r.fact === "NU" ? "#c62828" : "#555", fontWeight: 700, textAlign: "center" })} value={r.fact || ""} onChange={(e) => updLIV(i, "fact", e.target.value)}><option value=""></option><option>DA</option><option>NU</option></select></td><td style={td({ background: r.inc === "DA" ? "#c8e6c9" : r.inc === "NU" ? "#ffebee" : "#fff", textAlign: "center" })}><select style={sel({ color: r.inc === "DA" ? G : r.inc === "NU" ? "#c62828" : "#555", fontWeight: 700, textAlign: "center" })} value={r.inc || ""} onChange={(e) => updLIV(i, "inc", e.target.value)}><option value=""></option><option>DA</option><option>NU</option></select></td><td style={{ ...td({ background: rowBg }), overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}><input title={r.det || undefined} style={inp()} value={r.det || ""} onChange={(e) => updLIV(i, "det", e.target.value)} placeholder="..." /></td><td style={td({ textAlign: "center", padding: 3 })}><button onClick={() => delLIV(r.id)} style={{ background: "none", border: "none", cursor: "pointer", color: "#e53935", fontSize: 13 }}>✕</button></td></tr>); })}</tbody>
                ...
                <div style={{ maxWidth: 860 }}>
                  ...
                  <div style={{ display: "flex", gap: 14, flexWrap: "wrap" }}>
                    ...
                    <div style={{ flex: "1 1 380px", background: "#fff", border: "1px solid #e0e0e0", borderRadius: 10, padding: 16 }}>
                      ...
                      <div style={{ marginBottom: 10 }}><label style={FL}>Furnizor (cine aduce marfa) <span style={{ color: "#c62828" }}>*</span></label><div style={ACB}><AC value={ticNou.partener} options={partenerOpts} placeholder="Caută PF sau PJ..." onChange={(v) => { const f = pjList.find(x => x.denumire === v) || pfList.find(x => x.denumire === v); setTicNou((p) => ({ ...p, partener: v, partener_cui: f?.cod_fiscal || "" })); }} /></div></div>
                      <div style={{ display: "flex", gap: 8, marginBottom: 10 }}>
                      ...
                      <div style={{ display: "flex", gap: 8, marginBottom: 10 }}>
                        <div style={{ flex: 1 }}><label style={FL}>Nr. auto <span style={{ color: "#c62828" }}>*</span></label><div style={ACB}><AC value={ticNou.nr_masina} options={masiniOpts} placeholder="IF55KFT" onChange={(v) => setTicNou((p) => ({ ...p, nr_masina: v.toUpperCase() }))} /></div></div>
                      ...
                      <div style={{ marginBottom: 10 }}><label style={FL}>Material / Deșeu</label><div style={ACB}><select style={{ width: "100%", padding: "7px 9px", border: "1px solid #d5d5d5", borderRadius: 6, fontSize: 13, boxSizing: "border-box", cursor: "pointer" }} value={ticNou.material} onChange={(e) => setTicNou((p) => ({ ...p, material: e.target.value }))}><option value="">Selectează...</option>{PRODUSE_DYN.map((o) => <option key={o} value={o}>{o}</option>)}</select></div></div>
                      <div style={{ display: "flex", gap: 8 }}>
