                ...
                <div style={{ maxWidth: 860 }}>
                  ...
                  <div style={{ display: "flex", gap: 14, flexWrap: "wrap" }}>
                    ...
                    <div style={{ flex: "1 1 380px", background: "#fff", border: "1px solid #e0e0e0", borderRadius: 10, padding: 16 }}>
                      ...
                      <div style={{ marginBottom: 10 }}><label style={FL}>Furnizor (cine aduce marfa) <span style={{ color: "#c62828" }}>*</span></label><div style={{ ...ACB, display: "flex", alignItems: "center", gap: 4 }}><ACStrict value={ticNou.partener} options={partenerOpts} placeholder="Caută PF sau PJ..." onChange={(v) => { const f = pjList.find(x => x.denumire === v) || pfList.find(x => x.denumire === v); setTicNou((p) => ({ ...p, partener: v, partener_cui: f?.cod_fiscal || "" })); }} /><button onClick={() => searchAnafInto((v) => setTicNou((p) => ({ ...p, partener: v })))} title="Caută firma după CUI la ANAF" style={{ background: "none", border: "none", cursor: "pointer", fontSize: 13, flexShrink: 0, padding: 0 }}>🔍</button></div></div>
                      <div style={{ display: "flex", gap: 8, marginBottom: 10 }}>
                        ...
                        <div style={{ flex: 1 }}><label style={FL}>Transportator</label><div style={{ ...ACB, display: "flex", alignItems: "center", gap: 4 }}><ACStrict value={ticNou.transportator} options={transpOpts} placeholder="noi / PF / PJ" onChange={(v) => setTicNou((p) => ({ ...p, transportator: v }))} /><button onClick={() => searchAnafInto((v) => setTicNou((p) => ({ ...p, transportator: v })))} title="Caută firma după CUI la ANAF" style={{ background: "none", border: "none", cursor: "pointer", fontSize: 13, flexShrink: 0, padding: 0 }}>🔍</button></div></div>
                      </div>
                      <div style={{ display: "flex", gap: 8, marginBottom: 10 }}>
